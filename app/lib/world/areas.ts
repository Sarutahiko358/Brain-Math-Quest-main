...
export type Tile = 0 | 1 | 2 | 3 | 4 | 5; // 0草 1壁 2水 3町 4洞窟 5城
export const T = { Grass: 0, Wall: 1, Water: 2, Town: 3, Cave: 4, Castle: 5 } as const;
 ...
 */
function initializeGridWithBorders(): Tile[][] {
 ...
 */
function applyWaterPattern(g: Tile[][], pattern?: 'river' | 'lake' | 'none'): void {
  if (pattern === 'river') {
 ...
 */
function placeCustomWalls(g: Tile[][], wallPositions?: Vec[]): void {
 ...
 */
function addBorders(grid: Tile[][]): void {
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      if (r === 0 || c === 0 || r === ROWS - 1 || c === COLS - 1) {
        grid[r][c] = T.Wall;
 ...
 */
function addWaterPattern(grid: Tile[][], rand: () => number): void {
 ...
 */
function isProtectedPosition(r: number, c: number, dojoPos: Vec): boolean {
 ...
 */
function addRandomWalls(grid: Tile[][], rand: () => number, dojoPos: Vec): void {
 ...
 */
function addRandomCaves(grid: Tile[][], rand: () => number, dojoPos: Vec): void {
 ...
 */
export function createRandomMap(seed: number): Tile[][] {
 ...
 */
const DIR4: Vec[] = [{ r: 1, c: 0 }, { r: -1, c: 0 }, { r: 0, c: 1 }, { r: 0, c: -1 }];
 ...
 */
export function hasPath(map: Tile[][], from: Vec, to: Vec): boolean {
  const H = map.length;
  const W = map[0]?.length ?? 0;
  const inBounds = (r: number, c: number) => r >= 0 && c >= 0 && r < H && c < W;
...
export const AREAS: AreaInfo[] = [
  ...
  // NOTE: このエリア定義は後方互換性のために保持されています。
  // 実際の無限の回廊モードは gameMode === 'endless' で制御され、
  // ENDLESS_CONFIG (endless.ts) から設定が読み込まれます。
  {
    id: 10,
    name: "無限の回廊",
